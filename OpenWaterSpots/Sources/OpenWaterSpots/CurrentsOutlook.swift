...
public struct CurrentsOutlook: Sendable {

    /// Memberwise, spelled out because the synthesised one is internal
    /// and this type is read from the apps.
    public init(hours: [Hour], events: [Event] = [], source: Source,
                timeZone: TimeZone? = nil, staleAge: TimeInterval? = nil) {
        self.hours = hours
        self.events = events
        self.source = source
    ...
    public struct Hour: Identifiable, Sendable {
        ...
        /// Memberwise, spelled out because the synthesised one is internal
        /// and this type is read from the apps.
        public init(at: Date, speedKn: Double? = nil, directionDeg: Double? = nil) { self.at = at; self.speedKn = speedKn; self.directionDeg = directionDeg }
        public let at: Date
    ...
    public struct Event: Identifiable, Sendable {
        ...
        /// Memberwise, spelled out because the synthesised one is internal
        /// and this type is read from the apps.
        public init(at: Date, kind: Kind, speedKn: Double? = nil) { self.at = at; self.kind = kind; self.speedKn = speedKn }
        public enum Kind { case maxFlood, maxEbb, slack }
        public let at: Date
    ...
    public func hour(at instant: Date?) -> Hour? {
    ...
    public var nextEvent: Event? { events.first { $0.at > Date() } }
    ...
    public static func aligned(_ hours: [Hour], to axis: [Date]) -> [Hour] {
...
public struct CurrentStation: Identifiable, Hashable, Sendable {

    /// Memberwise, spelled out because the synthesised one is internal
    /// and this type is read from the apps.
    public init(id: String, bin: Int, name: String, metres: Double, coordinate: Geo.Coordinate, isHarmonic: Bool) { self.id = id; self.bin = bin; self.name = name; self.metres = metres; self.coordinate = coordinate; self.isHarmonic = isHarmonic }
...
public nonisolated enum CurrentPalette: Sendable {
    public static func color(for kn: Double) -> Color {
        switch kn {
...
extension OpenMeteo {
    ...
    public static func currents(at coordinate: Geo.Coordinate) async -> CurrentsOutlook {
    ...
    public static func parseModelCurrents(_ data: Data) -> CurrentsOutlook {
...
extension OpenMeteo {
    ...
    public static func marineAlong(_ coordinates: [Geo.Coordinate], hours: Int = 24,
                            pastHours: Int = 0)
...
extension TidesAndCurrents {
    ...
    public struct DistilledCurrent: Codable, Sendable {
        public let id: String
        public let name: String
    ...
    public static func currentStations(near coordinate: Geo.Coordinate, limit: Int = 3) async -> [CurrentStation] {
    ...
    public static func distillCurrentsIndex(_ data: Data) -> [DistilledCurrent] {
    ...
    public static func parseHourlyPredictions(_ data: Data) -> [CurrentsOutlook.Hour] {
        struct Payload: Decodable {
    ...
    public static func parseEventPredictions(_ data: Data) -> [CurrentsOutlook.Event] {
        struct Payload: Decodable {
            struct Root: Decodable { let cp: [Row]? }
            struct Row: Decodable {
                let Time: String
...
public enum Currents: Sendable {
    ...
    public static let stationRadius: Double = 15_000
